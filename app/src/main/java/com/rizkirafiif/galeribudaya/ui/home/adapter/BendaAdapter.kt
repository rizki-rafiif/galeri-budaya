...
class BendaAdapter(fragmentManager: FragmentManager):RecyclerView.Adapter<BendaAdapter.BendaViewHolder>() {
    var listBud = ArrayList<Budaya>()
    class BendaViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BendaViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_vp_content_home, parent, false)
        return BendaViewHolder(view)
    }

    override fun onBindViewHolder(holder: BendaViewHolder, position: Int) {
