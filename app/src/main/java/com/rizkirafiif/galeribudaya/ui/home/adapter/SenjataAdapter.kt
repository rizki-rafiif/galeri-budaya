...
class SenjataAdapter(fragmentManager: FragmentManager):RecyclerView.Adapter<SenjataAdapter.SenjataViewHolder>() {
    var listBud = ArrayList<Budaya>()
    class SenjataViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SenjataViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_vp_content_home, parent, false)
        return SenjataViewHolder(view)
    }

    override fun onBindViewHolder(holder: SenjataViewHolder, position: Int) {
